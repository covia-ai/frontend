     ...
     <Tooltip>
        <TooltipTrigger>
            ...
            (props.label && props.label !="") ? 
              ...
              (
                <div className="inline-flex items-center justify-center bg-muted text-muted-foreground border-1 border-border rounded-[11px] px-2 py-2 h-8 w-8 hover:bg-muted">
                   {props.compact &&  <Icon data-testid="btn-icon" size={16}></Icon> }
                   {!props.compact &&  <Icon data-testid="btn-icon" size={20}></Icon> }
                </div>
              )
