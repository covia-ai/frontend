...
export const AssetViewer = (props:any) => {
        ...
        useEffect(() => {
          ...
          venue.connect().then((venueObj) => {
              setVenue(venue);
              venueObj.getAsset(props.assetId).then(( asset:Asset) => {
                  console.log(asset)
                ...
                <div className="flex flex-row border-1 shadow-md rounded-md border-slate-200 w-11/12 mt-8 p-4 items-center justify-between">
                  <div className="flex flex-col w-1/2 border-r-2 border-slate-200 px-2">
                    ...
                    <div className="flex flex-row items-center space-x-2">
                      ...
                      <span><Link href={assetsMetadata?.metadata.license?.url} className="hover:underline hover:text-pink-400">{assetsMetadata?.metadata.license?.name}</Link></span>
                    </div> 
                    <div className="flex flex-row items-center space-x-2">
