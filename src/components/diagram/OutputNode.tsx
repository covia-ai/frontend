      ...
      {outputs.map((output, index) => (
           <CustomNodeToolTip key={index} posTop={getPosOutputForToolTip(index)} toolTip={output} position={Position.Left}
                    handle={<Handle
                      ...
                      >
                     {output.length <20 ? 
                     (<div className="text-[9px] text-foreground w-[5rem] flex flex-row pl-3 ">{output}</div>) :
                     (<div className="text-[9px] text-foreground -[5rem]  flex flex-row pl-3 ">{output.substring(0,18)+".."}</div>)
                     }
                     </Handle>}
          />
