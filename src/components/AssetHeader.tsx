...
export const AssetHeader = ({ assetsMetadata }: AssetHeaderProps) => {
  return (
    <div className="flex flex-col w-full items-center justify-center mb-2 mt-2 border-2 border-slate-200 rounded-md p-4">
      ...
      <p className="text-m mb-4 text-slate-600 w-3/4">{assetsMetadata?.metadata?.description}</p>
      ...
      <div className="flex flex-row-reverse space-x-4 space-x-reverse w-full justify-center">
        <div className="flex flex-row ">
          ...
          <span className="text-xs mr-1 font-mono">{assetsMetadata?.id}</span>
          <span>
            <Copy
              size={12}
              onClick={(e) => copyDataToClipBoard(assetsMetadata?.id, "AssetId copied to clipboard")}
